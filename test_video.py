...
class test_database(object):
    def __init__(self,model,test_path,is_dog = True):
        self.model = model.to(torch.device('cuda:1'))
    ...
    def predict_vid(self,idx):
        ...
        for i in tqdm(range(num_blocks)):
            ...
            frame_block = torch.tensor(frame_block.transpose(3,0,1,2), device = 'cuda:1')
            frame_block = frame_block.unsqueeze(0)
            if frame_block.shape[2]>0:
                num_blocks_nonempty += 1
                output,vect = self.predict_clip(frame_block)
                if idx==0 and i == 0:
                    print('First block')
                    torch.save(vect,"score.pt")
                    torch.save(frame_block,"first_block.pt")
    ...
    def predict_clip(self,sequence):
        ...
        return predict_class,predict_vect
...
state_dict = torch.load('./trained_models/dtdb_wts_R2p1D/dtdb_wts_R2p1D_DoG.pth', map_location='cuda:1')
